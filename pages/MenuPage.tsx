...
import { PlusCircle, Upload, X, Edit, Trash2, ArrowUp, ArrowDown, RotateCcw, Sparkles, Loader2, Save } from 'lucide-react';
...
// Helper to get all descendant IDs for filtering items
...
const MenuItemModal: React.FC<{
    ...
    const drinksCategoryId = useMemo(() => {
        // Heuristic: find a root category named "Bebidas"
        return categories.find(c => c.nombre.toLowerCase() === 'bebidas' && !c.parent_id)?.id;
    }, [categories]);
    ...
    // Recursive render for select options
    const renderCategoryOptions = (cats: any[], depth = 0) => {
        return cats.map(cat => (
            <React.Fragment key={cat.id}>
                <option value={cat.id}>
                    {'\u00A0'.repeat(depth * 4)}{cat.nombre}
                </option>
    ...
    // Effect to clean up recipe if category changes
    useEffect(() => {
        // Simple check: is this category a descendant of Drinks?
        let isDrinkCategory = false;
        let currentId = formData.category_id;
        while(currentId && drinksCategoryId) {
            if (currentId === drinksCategoryId) {
    ...
    }, [formData.category_id, formData.receta, drinksCategoryId, ingredients, categories]);

    const availableIngredients = useMemo(() => {
        let isDrinkCategory = false;
        let currentId = formData.category_id;
        
        // Check up the tree if it belongs to 'Bebidas'
        while(currentId && drinksCategoryId) {
            if (currentId === drinksCategoryId) {
                isDrinkCategory = true;
                break;
    ...
    }, [formData.category_id, ingredients, drinksCategoryId, categories]);
                                    ...
                                    return (
                                        <div key={index} className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-700/50 rounded-md">
                                            <select
                                                ...
                                                className="flex-1 min-w-0 px-2 py-1 text-sm bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 rounded-md"
                                            >
                                            ...
                                            <input
                                                ...
                                                step="0.01"
                                                placeholder="Cant."
                                                className="w-20 px-2 py-1 text-sm bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 rounded-md text-center"
                                            />
                                            <div className="w-24 flex flex-col text-xs">
                                                <span className="text-gray-500 dark:text-gray-400 font-medium">
                                                    {ing?.unidad || '-'}
                                                </span>
                                                {ing && (
                                                    <span className={`${ing.stock_actual <= ing.stock_minimo ? 'text-red-500 font-bold' : 'text-green-600'}`}>
                                                        Stock: {ing.stock_actual}
                                                    </span>
                                                )}
                                            </div>
                                            <button type="button" onClick={() => removeRecipeItem(index)} className="p-1 text-red-500 hover:text-red-700 rounded-full hover:bg-red-50 dark:hover:bg-red-900/20"><Trash2 className="h-4 w-4" /></button>
                                        </div>
                                    );
                                })}
                            </div>
...
export const MenuPage: React.FC = () => {
    ...
    const [activeRootCategory, setActiveRootCategory] = useState<string>('all');
    ...
    const rootCategories = useMemo(() => categories.filter(c => !c.parent_id).sort((a,b) => a.orden - b.orden), [categories]);

    useEffect(() => {
        if (activeRootCategory !== 'all' && !rootCategories.find(c => c.id === activeRootCategory)) {
            setActiveRootCategory('all');
        }
    }, [rootCategories, activeRootCategory]);

    // Get subcategories for the active root to display as chips
    const subCategories = useMemo(() => {
        if (activeRootCategory === 'all') return [];
        return categories.filter(c => c.parent_id === activeRootCategory).sort((a,b) => a.orden - b.orden);
    }, [categories, activeRootCategory]);

    const filteredItems = useMemo(() => {
        let items = allItemsForDisplay;

        if (activeRootCategory !== 'all') {
            // Get all descendents of the root
            const allowedCategoryIds = getDescendantIds(activeRootCategory, categories);
            items = items.filter(item => allowedCategoryIds.includes(item.category_id));
        }

        if (activeSubFilter) {
             const allowedSubIds = getDescendantIds(activeSubFilter, categories);
             items = items.filter(item => allowedSubIds.includes(item.category_id));
        }

        return items;
    }, [allItemsForDisplay, activeRootCategory, activeSubFilter, categories]);

    const handleRootTabClick = (id: string) => {
        setActiveRootCategory(id);
        setActiveSubFilter(null); // Reset sub filter when changing root
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-wrap justify-between items-center gap-4">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Menú</h1>
                {canEdit && (
                    <div className="flex items-center gap-2">
            ...
            <Card>
                ...
                <div className="border-b border-gray-200 dark:border-gray-700">
                    <nav className="-mb-px flex space-x-6 overflow-x-auto">
                        <button
                            onClick={() => handleRootTabClick('all')}
                            className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeRootCategory === 'all' ? 'border-orange-500 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                        >
                            Todos
                        </button>
                        {rootCategories.map(cat => (
                            <button
                                key={cat.id}
                                onClick={() => handleRootTabClick(cat.id)}
                                className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm ${activeRootCategory === cat.id ? 'border-orange-500 text-orange-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
                            >
                                {cat.nombre}
                            </button>
                        ))}
                ...
                {subCategories.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-4 pb-2">
                        ...
                        >
                            Todo en {categories.find(c => c.id === activeRootCategory)?.nombre}
                        </button>
